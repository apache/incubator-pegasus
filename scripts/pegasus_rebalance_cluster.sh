...
set_ok=`grep 'Succeed count: 0' /tmp/$UID.$PID.pegasus.rebalance_cluster.set_nfs_copy_rate_megabytes | wc -l`
if [ $set_ok == 1 ]; then
...
set_ok=`grep 'Succeed count: 0' /tmp/$UID.$PID.pegasus.rebalance_cluster.set_nfs_send_rate_megabytes | wc -l`
if [ $set_ok == 1 ]; then

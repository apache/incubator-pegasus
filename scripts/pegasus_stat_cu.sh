...
app_stat_failed=`grep "failed" $tmp_file | wc -l`
if [ $app_stat_failed -ne 0 ]; then
...
cat $app_stat_result | tail -n +3| head -n -1 | awk '{print $'$app_name_column',$'$data_size_column'}' > $app_info
...
do
  ...
  for filter in $*; do
    ...
    echo -e "use $cu_stat_table\nfull_scan -h prefix -x \"$filter\" -o $result_file" | ./run.sh shell -n $cluster&>$tmp_file

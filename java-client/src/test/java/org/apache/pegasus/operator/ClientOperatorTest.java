...
public class ClientOperatorTest {
  ...
  public void testSupportBackupRequest() {
    ClientOperator op =
        new RRDBMultiGetOperator(new gpid(1, 1), "test", new multi_get_request(), 0);
    Assert.assertTrue(op.supportBackupRequest());

    op = new RRDBGetOperator(new gpid(1, 1), "test", new blob(), 0);
    Assert.assertTrue(op.supportBackupRequest());

    op = new RRDBTTLOperator(new gpid(1, 1), "test", new blob(), 0);
    Assert.assertTrue(op.supportBackupRequest());

    op = new RRDBPutOperator(new gpid(1, 1), "test", new update_request(), 0);
    Assert.assertFalse(op.supportBackupRequest());

    op = new RRDBCheckAndMutateOperator(new gpid(1, 1), "test", new check_and_mutate_request(), 0);
    Assert.assertFalse(op.supportBackupRequest());
  }
}

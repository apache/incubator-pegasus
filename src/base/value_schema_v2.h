...
public:
    ...
    std::unique_ptr<value_field> extract_field(string_view value, value_field_type type) override;
    blob extract_user_data(std::string &&value) override;
...
private:
    std::unique_ptr<value_field> extract_timestamp(string_view value);
    std::unique_ptr<value_field> extract_time_tag(string_view value);

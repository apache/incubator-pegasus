...
// T may be std::string or blob.
// data is copied into 'key'.
template <typename T>
void pegasus_generate_key(blob &key, const T &hash_key, const T &sort_key)
{
    ...
    *((int16_t *)buf.get()) = endian::hton((uint16_t)hash_key_len);
...
template <typename T>
void pegasus_generate_next_blob(blob &next, const T &hash_key)
{
    CHECK_LT(hash_key.length(), UINT16_MAX);

    int hash_key_len = hash_key.length();
    std::shared_ptr<char> buf(utils::make_shared_array<char>(hash_key_len + 2));
...
// data is copied into 'next'.
template <typename T>
void pegasus_generate_next_blob(blob &next, const T &hash_key, const T &sort_key)
{
    blob buf;
...
// no data copied.
inline void pegasus_restore_key(const blob &key, blob &hash_key, blob &sort_key)
{
    CHECK_GE(key.length(), 2);

    // hash_key_len is in big endian
    uint16_t hash_key_len = endian::ntoh(*(uint16_t *)(key.data()));
...
inline void pegasus_restore_key(const blob &key, std::string &hash_key, std::string &sort_key)
{
    CHECK_GE(key.length(), 2);
...
inline uint64_t pegasus_key_hash(const T &key)
{
    ...
    if (hash_key_len > 0) {
        ...
        return utils::crc64_calc(key.data() + 2, hash_key_len, 0);
    } else {
        // hash_key_len == 0, compute hash from sort_key
        return utils::crc64_calc(key.data() + 2, key.size() - 2, 0);
    }
}

/// Calculate hash value from hash key.
inline uint64_t pegasus_hash_key_hash(const blob &hash_key)
{
    return utils::crc64_calc(hash_key.data(), hash_key.length(), 0);
}

...
public class PegasusScanner implements PegasusScannerInterface {
    ...
    public void close() {
        if (_context >= CONTEXT_ID_VALID_MIN) {
            try {
                rrdb_clear_scanner_operator op = new rrdb_clear_scanner_operator(_gpid, _table.getTableName(), _context);
                _table.operate(op, 0);
            } catch (Throwable e) {
                // ignore
            }
    ...
    private void asyncStartScan() {
        ...
        rrdb_get_scanner_operator op = new rrdb_get_scanner_operator(_gpid, _table.getTableName(), request);

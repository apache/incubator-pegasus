...
public final class MetricsManager {
    ...
    public static final void initFromHost(String host, String tag, int reportIntervalSec) {
        synchronized (logger) {
            if (started) {
                logger.warn("perf counter system has started with host({}), tag({}), interval({}), " +
                        "skip this init with host({}), tag({}), interval(){}",
                        MetricsManager.host, MetricsManager.tag, MetricsManager.reportIntervalSecs,
                        host, tag, reportIntervalSec);
                return;
            }

            logger.info("init metrics with host({}), tag({}), interval({})",
                    host, tag, reportIntervalSec);

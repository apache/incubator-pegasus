...
public class Tools {
    ...
    public static <T> T waitUninterruptable(FutureTask<T> task, long millseconds) throws ExecutionException {
        long current = System.currentTimeMillis();
        while (millseconds >= 0) {
            try {
    ...
    private final static long epoch_begin = 1451606400; // seconds since 2016.01.01-00:00:00 GMT
        ...
        try {
            ...
            while(e.hasNext()) {
                ...
                while(var2.hasNext()) {
                    ...
                    if(addr.isSiteLocalAddress() && !addr.isLoopbackAddress() && !(addr instanceof Inet6Address)

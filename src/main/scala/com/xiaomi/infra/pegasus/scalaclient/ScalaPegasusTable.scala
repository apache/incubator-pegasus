...
import com.xiaomi.infra.pegasus.client.{PException, PegasusTableInterface => ITable, HashKeyData => PHashKeyData}
...
trait ScalaPegasusTable extends PegasusUtil {
    ...
    def exists[H, S](hashKey: H, sortKey: S, timeout: Duration = 0 milli)
            (implicit hSer: SER[H], sSer: SER[S]): Boolean = {
    ...
    def get[H, S](hashKey: H, sortKey: S, timeout: Duration = 0 milli)
            (implicit hSer: SER[H], sSer: SER[S]): PegasusResult = {
    ...
    def batchGet[H, S](keys: Seq[PegasusKey[H, S]], timeout: Duration = 0 milli)
            (implicit hSer: SER[H], sSer: SER[S]): PegasusResultList = {
    ...
    def batchGet2[H, S](keys: Seq[PegasusKey[H, S]], timeout: Duration = 0 milli)
            (implicit hSer: SER[H], sSer: SER[S]): BatchGetResult[Array[Byte]] = {
        ...
        BatchGetResult(count, result.toList.map(pairToEither[PException, Array[Byte]]))
    }

    @throws[PException]
    def multiGet[H, S](hashKey: H, sortKeys: Seq[S], maxFetchCount: Int = 100, maxFetchSize: Int = 1000000, timeout: Duration = 0 milli)
            (implicit hSer: SER[H], sSer: SER[S]): MultiGetResult[S, Array[Byte]] = {
        val result = table.multiGet(hashKey, sortKeys, maxFetchCount, maxFetchSize, timeout)
    ...
    def multiGetRange[H, S](hashKey: H, startSortKey: S, stopSortKey: S, options: Options.MultiGet,
            ...
            (implicit hSer: SER[H], sSer: SER[S]): MultiGetResult[S, Array[Byte]] = {
        val result = table.multiGet(hashKey, startSortKey, stopSortKey, options, maxFetchCount, maxFetchSize, timeout)
        convertMultiGetResult(result)
    }

    @throws[PException]
    def batchMultiGet[H, S](keys: Seq[(H, Seq[S])], timeout: Duration = 0 milli)
            (implicit hSer: SER[H], sSer: SER[S]): List[HashKeyData[H, S, Array[Byte]]] = {
    ...
    def batchMultiGet2[H, S](keys: Seq[(H, Seq[S])], timeout: Duration = 0 milli)
            (implicit hSer: SER[H], sSer: SER[S]): BatchMultiGetResult[H, S, Array[Byte]] = {
        ...
        val either = result.map(p => pairToEither(p, convertToHashKeyData[H, S, Array[Byte]])).toList
    ...
    def multiGetSortKeys[H](hashKey: H, maxFetchCount: Int = 100, maxFetchSize: Int = 1000000, timeout: Duration = 0 milli)
            (implicit hSer: SER[H]) = {
        val result = table.multiGetSortKeys(hashKey, maxFetchCount, maxFetchSize, timeout)
    ...
    @throws[PException]
    def batchSet[H, S, V](items: Seq[SetItem[H, S, V]], timeout: Duration = 0 milli)
            (implicit hSer: SER[H], sSer: SER[S], vSer: SER[V]) = {
        table.batchSet(items.map(convertSetItem[H, S, V]), timeout)
    }

    @throws[PException]
    def batchSet2[H, S, V](items: Seq[SetItem[H, S, V]], timeout: Duration = 0 milli)
            (implicit hSer: SER[H], sSer: SER[S], vSer: SER[V]): BatchOpResult = {
    ...
    def multiSet[H, S, V](hashKey: H, values: Seq[(S, V)], ttl: Duration = 0 second, timeout: Duration = 0 milli)
            (implicit hSer: SER[H], sSer: SER[S], vSer: SER[V]) = {
        table.multiSet(hashKey, values.map(tupleToBytesPair(_)), ttl.toSeconds.toInt, timeout)
    }

    @throws[PException]
    def batchMultiSet[H, S, V](items: Seq[HashKeyData[H, S, V]], ttl: Duration = 0 second, timeout: Duration = 0 milli)
            (implicit hSer: SER[H], sSer: SER[S], vSer: SER[V]) = {
        table.batchMultiSet(items.map(convertHashKeyData[H, S, V]), ttl.toSeconds.toInt, timeout)
    }

    @throws[PException]
    def batchMultiSet2[H, S, V](items: Seq[HashKeyData[H, S, V]], ttl: Duration = 0 second, timeout: Duration = 0 milli)
            (implicit hSer: SER[H], sSer: SER[S], vSer: SER[V]): BatchOpResult = {
        val result = new util.ArrayList[PException](items.size)
        val count = table.batchMultiSet2(items.map(convertHashKeyData[H, S, V]), ttl.toSeconds.toInt, result, timeout)
        BatchOpResult(count, result.toList.map(Option.apply))
    }

...
import com.xiaomi.infra.pegasus.client.{PException, PegasusClientInterface => IClient}
...
trait ScalaPegasusClient extends PegasusUtil {
    ...
    @throws[PException]
    def openTable(table: String) = new ScalaPegasusTableImpl(client.openTable(table))

    @throws[PException]
    def openAsyncTable(table: String) = new ScalaPegasusAsyncTableImpl(client.openTable(table))
    ...
    @throws[PException]
    def sortKeyCount[H](table: String, hashKey: H)(implicit hSer: SER[H]) = getTable(table).sortKeyCount(hashKey)
    ...
    @throws[PException]
    def multiGetRange[H, S](table: String, hashKey: H, startSortKey: S, stopSortKey: S, options: Options.MultiGet,
            maxFetchCount: Int = 100, maxFetchSize: Int = 1000000)
            (implicit hSer: SER[H], sSer: SER[S]): MultiGetResult[S, Array[Byte]] = {
        getTable(table).multiGetRange(hashKey, startSortKey, stopSortKey, options, maxFetchCount, maxFetchSize)
    }
    ...
    @throws[PException]
    def multiGetSortKeys[H](table: String, hashKey: H, maxFetchCount: Int = 100, maxFetchSize: Int = 1000000)
            (implicit hSer: SER[H]) = {
        getTable(table).multiGetSortKeys(hashKey, maxFetchCount, maxFetchSize)
    }

    @throws[PException]
    def set[H, S, V](table: String, hashKey: H, sortKey: S, value: V, ttl: Duration = 0 second)

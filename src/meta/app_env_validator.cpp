...
bool app_env_validator::validate_app_envs(const std::map<std::string, std::string> &envs)
{
    ...
    for (const auto &[key, value] : envs) {
        if (!ContainsKey(replica_envs::ROCKSDB_STATIC_OPTIONS, key) &&
            !ContainsKey(replica_envs::ROCKSDB_DYNAMIC_OPTIONS, key)) {
            continue;
        }
                                         ...
                                         std::string &hint_message)
{
    ...
    const auto *func = FindOrNull(_validator_funcs, env_name);
    if (func == nullptr) {
...
{
    const auto *type_str = FindOrNull(ValueType2String, type);

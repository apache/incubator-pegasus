...
void info_collector::on_app_stat()
{
    ...
    if (get_app_stat(&_shell_context, "", rows)) {
        ...
        for (int i = 0; i < rows.size() - 1; ++i) {
            ...
            all.recent_throttling_delay_count += row.recent_throttling_delay_count;
            all.recent_throttling_reject_count += row.recent_throttling_reject_count;
        ...
        for (int i = 0; i < rows.size(); ++i) {
            ...
            counters->recent_throttling_delay_count->set(row.recent_throttling_delay_count);
            counters->recent_throttling_reject_count->set(row.recent_throttling_reject_count);
...
#define INIT_COUNER(type)                                                                          \
    ...
    INIT_COUNER(recent_throttling_delay_count);
    INIT_COUNER(recent_throttling_reject_count);

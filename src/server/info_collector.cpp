...
void info_collector::on_app_stat()
{
    ...
    for (int i = 0; i < rows.size() - 1; ++i) {
        ...
        all.recent_table_level_abnormal_count += row.recent_table_level_abnormal_count;
    }
    ...
    for (int i = 0; i < rows.size(); ++i) {
        ...
        counters->recent_table_level_abnormal_count->set(row.recent_table_level_abnormal_count);
    }
...
#define INIT_COUNTER(name)                                                                         \
    ...
    INIT_COUNTER(recent_table_level_abnormal_count);

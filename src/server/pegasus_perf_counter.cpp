...
class pegasus_perf_counter_number_atomic : public perf_counter
{
public:
    pegasus_perf_counter_number_atomic(const char *app,
                                       const char *section,
                                       const char *name,
...
class pegasus_perf_counter_volatile_number_atomic : public pegasus_perf_counter_number_atomic
{
public:
    pegasus_perf_counter_volatile_number_atomic(const char *app,
                                                const char *section,
                                                const char *name,
                                                dsn_perf_counter_type_t type,
                                                const char *dsptr)
        : pegasus_perf_counter_number_atomic(app, section, name, type, dsptr)
    {
    }
    ~pegasus_perf_counter_volatile_number_atomic(void) {}
...
class pegasus_perf_counter_rate_atomic : public perf_counter
{
public:
    pegasus_perf_counter_rate_atomic(const char *app,
                                     const char *section,
                                     const char *name,
                                     dsn_perf_counter_type_t type,
                                     const char *dsptr)
...
class pegasus_perf_counter_number_percentile_atomic : public perf_counter
{
public:
    pegasus_perf_counter_number_percentile_atomic(const char *app,
                                                  const char *section,
                                                  const char *name,
                                                  dsn_perf_counter_type_t type,
                                                  const char *dsptr)
        : perf_counter(app, section, name, type, dsptr), _tail(0)
    {
        ...
        this->add_ref();
        _timer->async_wait(std::bind(&pegasus_perf_counter_number_percentile_atomic::on_timer,
                                     this,
                                     _timer,
                                     std::placeholders::_1));
    }
                  ...
                  const boost::system::error_code &ec)
    {
        ...
        if (!ec) {
            ...
            if (this->get_count() > 1) {
                ...
                this->add_ref();
                timer->async_wait(
                    std::bind(&pegasus_perf_counter_number_percentile_atomic::on_timer,
                              this,
                              timer,
                              std::placeholders::_1));
            }
...
perf_counter *pegasus_perf_counter_factory(const char *app,
                                           const char *section,
                                           const char *name,
                                           dsn_perf_counter_type_t type,
                                           const char *dsptr)
{
    if (type == dsn_perf_counter_type_t::COUNTER_TYPE_NUMBER)
        return new pegasus_perf_counter_number_atomic(app, section, name, type, dsptr);
    else if (type == dsn_perf_counter_type_t::COUNTER_TYPE_VOLATILE_NUMBER)
        return new pegasus_perf_counter_volatile_number_atomic(app, section, name, type, dsptr);
    else if (type == dsn_perf_counter_type_t::COUNTER_TYPE_RATE)
        return new pegasus_perf_counter_rate_atomic(app, section, name, type, dsptr);
    else if (type == dsn_perf_counter_type_t::COUNTER_TYPE_NUMBER_PERCENTILES)
        return new pegasus_perf_counter_number_percentile_atomic(app, section, name, type, dsptr);
    else {

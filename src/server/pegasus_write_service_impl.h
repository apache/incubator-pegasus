...
inline int get_cluster_id_if_exists()
{
    ...
    static auto cluster_id_res =
        replication::get_duplication_cluster_id(get_current_cluster_name());
    static uint64_t cluster_id = cluster_id_res.is_ok() ? cluster_id_res.get_value() : 0;
    return cluster_id;
}

inline error_code get_external_files_path(const std::string &bulk_load_dir,
                                          const bool verify_before_ingest,
                                          const replication::bulk_load_metadata &metadata,
                                          /*out*/ std::vector<std::string> &files_path)
{
    for (const auto &f_meta : metadata.files) {
        const auto &file_name = utils::filesystem::path_combine(bulk_load_dir, f_meta.name);
    ...
    return files_path.size() == metadata.files.size() ? ERR_OK : ERR_WRONG_CHECKSUM;
}

class pegasus_write_service::impl : public replication::replica_base
{
public:
    ...
    {
        int err = _rocksdb_wrapper->write_batch_put(decree, string_view(), string_view(), 0);
        auto cleanup = defer([this]() { _rocksdb_wrapper->clear_up_write_batch(); });
    ...
    int incr(int64_t decree, const apps::incr_request &update, apps::incr_response &resp)
    {
        ...
        string_view raw_key(update.key.data(), update.key.length());
        int64_t new_value = 0;
        ...
        if (!get_ctx.found) {
            ...
            if (old_value.length() == 0) {
                ...
                if (!buf2int64(old_value, old_value_int)) {
                    // invalid old value
                      ...
                      apps::check_and_set_response &resp)
    {
        ...
        string_view check_raw_key(check_key.data(), check_key.length());
        ...
        if (passed) {
            ...
            resp.error = _rocksdb_wrapper->write_batch_put(decree, string_view(), string_view(), 0);
        }
                         ...
                         apps::check_and_mutate_response &resp)
    {
        ...
        for (int i = 0; i < update.mutate_list.size(); ++i) {
            ...
            if (mu.operation != apps::mutate_operation::MO_PUT &&
                mu.operation != apps::mutate_operation::MO_DELETE) {
                LOG_ERROR_PREFIX("invalid argument for check_and_mutate: decree = {}, error = "
        ...
        if (passed) {
            for (auto &m : update.mutate_list) {
                ...
                if (m.operation == apps::mutate_operation::MO_PUT) {
                    resp.error = _rocksdb_wrapper->write_batch_put(
                        decree, key, m.value, static_cast<uint32_t>(m.set_expire_ts_seconds));
                } else {
                    CHECK_EQ(m.operation, apps::mutate_operation::MO_DELETE);
    ...
    error_code ingest_files(const int64_t decree,
...
private:
    void clear_up_batch_states(int64_t decree, int err)
    {
        if (!_update_responses.empty()) {
            ...
            resp.decree = decree;
            resp.server = _primary_address;
            for (apps::update_response *uresp : _update_responses) {
                *uresp = resp;
            }
    ...
    static blob composite_raw_key(string_view hash_key, string_view sort_key)
    {
        blob raw_key;
    ...
    static bool is_check_type_supported(apps::cas_check_type::type check_type)
    {
        return check_type >= apps::cas_check_type::CT_NO_CHECK &&
               check_type <= apps::cas_check_type::CT_VALUE_INT_GREATER;
    }
    ...
    bool validate_check(int64_t decree,
                        apps::cas_check_type::type check_type,
                        const blob &check_operand,
                        bool value_exist,
                        const blob &value,
                        bool &invalid_argument)
    {
        ...
        switch (check_type) {
        case apps::cas_check_type::CT_NO_CHECK:
            return true;
        case apps::cas_check_type::CT_VALUE_NOT_EXIST:
            return !value_exist;
        case apps::cas_check_type::CT_VALUE_NOT_EXIST_OR_EMPTY:
            return !value_exist || value.length() == 0;
        case apps::cas_check_type::CT_VALUE_EXIST:
            return value_exist;
        case apps::cas_check_type::CT_VALUE_NOT_EMPTY:
            return value_exist && value.length() != 0;
        case apps::cas_check_type::CT_VALUE_MATCH_ANYWHERE:
        case apps::cas_check_type::CT_VALUE_MATCH_PREFIX:
        case apps::cas_check_type::CT_VALUE_MATCH_POSTFIX: {
            if (!value_exist)
                return false;
            ...
            if (check_type == apps::cas_check_type::CT_VALUE_MATCH_ANYWHERE) {
                return string_view(value).find(check_operand) != string_view::npos;
            } else if (check_type == apps::cas_check_type::CT_VALUE_MATCH_PREFIX) {
                return utils::mequals(value.data(), check_operand.data(), check_operand.length());
            } else { // check_type == apps::cas_check_type::CT_VALUE_MATCH_POSTFIX
                return utils::mequals(value.data() + value.length() - check_operand.length(),
                                      check_operand.data(),
                                      check_operand.length());
            }
        }
        case apps::cas_check_type::CT_VALUE_BYTES_LESS:
        case apps::cas_check_type::CT_VALUE_BYTES_LESS_OR_EQUAL:
        case apps::cas_check_type::CT_VALUE_BYTES_EQUAL:
        case apps::cas_check_type::CT_VALUE_BYTES_GREATER_OR_EQUAL:
        case apps::cas_check_type::CT_VALUE_BYTES_GREATER: {
            if (!value_exist)
                return false;
            int c = string_view(value).compare(string_view(check_operand));
            if (c < 0) {
                return check_type <= apps::cas_check_type::CT_VALUE_BYTES_LESS_OR_EQUAL;
            } else if (c == 0) {
                return check_type >= apps::cas_check_type::CT_VALUE_BYTES_LESS_OR_EQUAL &&
                       check_type <= apps::cas_check_type::CT_VALUE_BYTES_GREATER_OR_EQUAL;
            } else { // c > 0
                return check_type >= apps::cas_check_type::CT_VALUE_BYTES_GREATER_OR_EQUAL;
            }
        }
        case apps::cas_check_type::CT_VALUE_INT_LESS:
        case apps::cas_check_type::CT_VALUE_INT_LESS_OR_EQUAL:
        case apps::cas_check_type::CT_VALUE_INT_EQUAL:
        case apps::cas_check_type::CT_VALUE_INT_GREATER_OR_EQUAL:
        case apps::cas_check_type::CT_VALUE_INT_GREATER: {
            if (!value_exist)
                return false;
            int64_t check_value_int;
            if (!buf2int64(value, check_value_int)) {
                // invalid check value
            ...
            if (!buf2int64(check_operand, check_operand_int)) {
            ...
            if (check_value_int < check_operand_int) {
                return check_type <= apps::cas_check_type::CT_VALUE_INT_LESS_OR_EQUAL;
            } else if (check_value_int == check_operand_int) {
                return check_type >= apps::cas_check_type::CT_VALUE_INT_LESS_OR_EQUAL &&
                       check_type <= apps::cas_check_type::CT_VALUE_INT_GREATER_OR_EQUAL;
            } else { // check_value_int > check_operand_int
                return check_type >= apps::cas_check_type::CT_VALUE_INT_GREATER_OR_EQUAL;
            }
        }
        default:
...
private:
    ...
    std::vector<apps::update_response *> _update_responses;
};

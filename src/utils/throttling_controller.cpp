                                       ...
                                       /*out*/ std::string &hint_message)
{
    ...
    // Check overflow.
    uint64_t result;
    if (__builtin_mul_overflow(units, unit_multiplier, &result)) {
...
bool throttling_controller::parse_from_env(const std::string &arg,
                                           ...
                                           /*out*/ std::string &hint_message)
{
    ...
    for (const auto &sarg : sargs) {
        ...
        if (parse_from_env(sarg, "delay", units, ms, hint_message)) {
            if (delay_parsed) {
        ...
        if (parse_from_env(sarg, "reject", units, ms, hint_message)) {
            if (reject_parsed) {
                                           ...
                                           std::string &old_env_value)
{
    ...
    _delay_units = delay_units / partition_count + 1;
    _delay_ms = delay_ms;
    _reject_units = reject_units / partition_count + 1;

...
typename std::enable_if<
    std::is_same<typename Collection::key_type, typename Collection::value_type>::value,
    bool>::type
...
{
    return collection->insert_or_assign(key, std::forward<typename Collection::mapped_type>(value))
        .second;
}
...
void MergeTokenCounts(Collection *const count_map, const Collection &counts_to_add)
{
    for (const auto &value_and_count : counts_to_add) {
        typename Collection::mapped_type &value =
            LookupOrInsert(count_map, value_and_count.first, typename Collection::mapped_type());
...
LookupOrInsertNewSharedPtr(Collection *const collection, const typename Collection::key_type &key)
{
...
typename Collection::mapped_type &LookupOrInsertNewSharedPtr(
    Collection *const collection, const typename Collection::key_type &key, const Args &...args)
{
    typedef typename Collection::mapped_type SharedPtr;
    typedef typename Collection::mapped_type::element_type Element;
    std::pair<typename Collection::iterator, bool> ret =

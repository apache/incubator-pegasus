...
void zookeeper_session::global_string_completion(int rc, const char *name, const void *data)
{
    ...
    ddebug("%s, rc(%s), name(%s), input path(%s)", __PRETTY_FUNCTION__, zerror(rc), name, op_ctx->_input._path.c_str());
...
{
    COMPLETION_INIT(rc, data);
    ddebug("%s, rc(%s), input path(%s)", __PRETTY_FUNCTION__, zerror(rc), op_ctx->_input._path.c_str());
...
{
    COMPLETION_INIT(rc, data);
    ddebug("%s, rc(%s), input path(%s), child count(%d)", __PRETTY_FUNCTION__, zerror(rc), op_ctx->_input._path.c_str(), strings->count);

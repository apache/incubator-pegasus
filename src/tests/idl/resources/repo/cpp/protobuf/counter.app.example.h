        ...
        public:
            ...
            virtual void on_test_timer()
            {
                ...
                // test for service 'counter'
                {
                    //sync:
                    count_op args;
                    args.set_operand(TEST_ADD_ARGS_OPERAND);
                    args.set_name(TEST_ADD_ARGS_NAME);
                    auto result = _counter_client->add_sync(args);
                    if (result.first == ::dsn::ERR_OK)
                    {
                        bool ok = result.second.value() == TEST_ADD_RESULT;
                ...
                }
                {
                    //sync:
                    count_name args;
                    args.set_name(TEST_READ_ARGS_NAME);
                    auto result = _counter_client->read_sync(args);
                    if (result.first == ::dsn::ERR_OK)
                    {
                        bool ok = result.second.value() == TEST_READ_RESULT;

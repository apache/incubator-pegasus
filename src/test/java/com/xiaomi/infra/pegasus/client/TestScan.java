...
public class TestScan {
        ...
        for (int i = scanners.size() - 1; i >= 0; i--)
        {
            ...
            while (true) {
                ...
                try {
                    ...
                    if (pair == null)
                        break;
                    checkAndPut(data, new String(pair.getLeft().getLeft()), new String(pair.getLeft().getRight()),
    ...
    public void testConcurrentCallAsyncScan() throws PException {
        ...
        for (int idx = 0; idx < batchSizes.length; idx ++) {
            ...
            System.out.println("start to scan " + totalKvSize + " kv-pairs with scan batch size " + batchSize);

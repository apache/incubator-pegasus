...
TEST_P(IncrTest, Overflow)
{
    ...
    int64_t new_value_int;
    ASSERT_EQ(PERR_INVALID_ARGUMENT,
              _client->incr("incr_test_overflow", "", std::numeric_limits<int64_t>::max(), new_value_int));
...
TEST_P(IncrTest, Underflow)
{
    ...
    int64_t new_value_int;
    ASSERT_EQ(PERR_INVALID_ARGUMENT,
              _client->incr("incr_test_underflow", "", std::numeric_limits<int64_t>::min(), new_value_int));
    ASSERT_EQ(-1, new_value_int);
...
INSTANTIATE_TEST_SUITE_P(AtomicWriteTest, IncrTest, testing::ValuesIn(generate_atomic_write_cases()));

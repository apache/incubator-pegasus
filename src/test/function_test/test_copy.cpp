...
static void verify_data()
{
    ...
    for (auto it1 = data.begin(), it2 = base_data.begin();; ++it1, ++it2) {
        if (it1 == data.end()) {
            ASSERT_EQ(base_data.end(), it2) << "Only in base_data: hash_key=" << it2->first;
            break;
        }
        ASSERT_NE(base_data.end(), it2) << "Only in data: hash_key=" << it1->first;
        ASSERT_EQ(it1->first, it2->first) << "Diff: data_hash_key=" << it1->first
                                          << ", base_data_hash_key=" << it2->first;

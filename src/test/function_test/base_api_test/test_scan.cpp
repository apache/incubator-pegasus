...
public:
    void SetUp() override
    {
        ...
        pg_client_ = pegasus_client_factory::get_client(cluster_name_.c_str(), app_name_.c_str());
        ASSERT_TRUE(pg_client_ != nullptr);
    ...
    void fill_database()
    {
        ...
        while (base[expected_hash_key].size() < 1000) {
            ...
            value = random_string();
            int ret = pg_client_->set(expected_hash_key, sort_key, value);
            ASSERT_EQ(PERR_OK, ret) << "Error occurred when set, hash_key=" << expected_hash_key
                                    << ", sort_key=" << sort_key
                                    << ", error=" << pg_client_->get_error_string(ret);
            i++;
        ...
        while (base.size() < 501) {
            ...
            while (base[hash_key].size() < 10) {
                ...
                value = random_string();
                int ret = pg_client_->set(hash_key, sort_key, value);
        ...
        while (base.size() < 1001) {
            ...
            while (base[hash_key].size() < 10) {
                ...
                int ret = pg_client_->set(hash_key, sort_key, value, 5000, ttl_seconds, nullptr);
                i++;
                ASSERT_EQ(PERR_OK, ret) << "Error occurred when set, hash_key=" << hash_key
                                        << ", sort_key=" << sort_key
                                        << ", error=" << pg_client_->get_error_string(ret);
...
TEST_F(scan, OVERALL_COUNT_ONLY)
{
    ...
    int ret = pg_client_->get_unordered_scanners(3, options, scanners);
...
TEST_F(scan, ALL_SORT_KEY)
{
    ...
    int ret = pg_client_->get_scanner(expected_hash_key, "", "", options, scanner);
...
TEST_F(scan, BOUND_INCLUSIVE)
{
    ...
    std::map<std::string, std::string> data;
    pegasus_client::pegasus_scanner *scanner = nullptr;
    int ret = pg_client_->get_scanner(expected_hash_key, start, stop, options, scanner);
    ASSERT_EQ(PERR_OK, ret) << "Error occurred when getting scanner. error="
                            << pg_client_->get_error_string(ret);
    ASSERT_NE(nullptr, scanner);
...
TEST_F(scan, ONE_POINT)
{
    ...
    options.stop_inclusive = true;
    pegasus_client::pegasus_scanner *scanner = nullptr;
    int ret = pg_client_->get_scanner(expected_hash_key, start, start, options, scanner);
    ASSERT_EQ(PERR_OK, ret) << "Error occurred when getting scanner. error="
                            << pg_client_->get_error_string(ret);
    ASSERT_NE(nullptr, scanner);
    ...
    ASSERT_EQ(PERR_OK, ret) << "Error occurred when scan. error="
                            << pg_client_->get_error_string(ret);
    ASSERT_EQ(expected_hash_key, hash_key);
...
TEST_F(scan, VOID_SPAN)
{
    ...
    options.stop_inclusive = true;
    pegasus_client::pegasus_scanner *scanner = nullptr;
    int ret = pg_client_->get_scanner(expected_hash_key, stop, start, options, scanner);
    ASSERT_EQ(PERR_OK, ret) << "Error occurred when getting scanner. error="
                            << pg_client_->get_error_string(ret);
    ASSERT_NE(nullptr, scanner);
...
TEST_F(scan, REQUEST_EXPIRE_TS)
{
    ...
    int ret = pg_client_->get_unordered_scanners(3, options, raw_scanners);
            ...
            scanner->async_next([&](int err,
                ...
                if (err == pegasus::PERR_OK) {
                    ...
                    ASSERT_TRUE(false) << "Error occurred when scan. error="
                                       << pg_client_->get_error_string(err);
                }
                op_completed.notify();
            });
            op_completed.wait();
...
TEST_F(scan, ITERATION_TIME_LIMIT)
{
    ...
    auto response = ddl_client_->set_app_envs(
        pg_client_->get_app_name(), {ROCKSDB_ITERATION_THRESHOLD_TIME_MS}, {std::to_string(1)});
    ...
    int64_t count = 0;
    int ret = pg_client_->sortkey_count(expected_hash_key, count);
    ASSERT_EQ(0, ret);
    ASSERT_EQ(count, -1);

    // set iteration threshold to 100ms
    response = ddl_client_->set_app_envs(
        pg_client_->get_app_name(), {ROCKSDB_ITERATION_THRESHOLD_TIME_MS}, {std::to_string(100)});
    ASSERT_EQ(true, response.is_ok());
    ASSERT_EQ(dsn::ERR_OK, response.get_value().err);
    // wait envs to be synced.
    std::this_thread::sleep_for(std::chrono::seconds(30));

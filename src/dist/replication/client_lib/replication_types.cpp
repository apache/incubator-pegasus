...
void balancer_proposal_request::__set_from_server(const  ::dsn::rpc_address& val) {
  this->from_server = val;
}

void balancer_proposal_request::__set_to_server(const  ::dsn::rpc_address& val) {
  this->to_server = val;
}
...
uint32_t balancer_proposal_request::read(::apache::thrift::protocol::TProtocol* iprot) {
  ...
  while (true)
  {
    ...
    switch (fid)
    {
      ...
      case 3:
        if (ftype == ::apache::thrift::protocol::T_STRUCT) {
          xfer += this->from_server.read(iprot);
          this->__isset.from_server = true;
        } else {
          xfer += iprot->skip(ftype);
        }
        break;
      case 4:
        if (ftype == ::apache::thrift::protocol::T_STRUCT) {
          xfer += this->to_server.read(iprot);
          this->__isset.to_server = true;
        } else {
          xfer += iprot->skip(ftype);
        }
        break;
...
uint32_t balancer_proposal_request::write(::apache::thrift::protocol::TProtocol* oprot) const {
  ...
  xfer += oprot->writeFieldBegin("from_server", ::apache::thrift::protocol::T_STRUCT, 3);
  xfer += this->from_server.write(oprot);
  xfer += oprot->writeFieldEnd();

  xfer += oprot->writeFieldBegin("to_server", ::apache::thrift::protocol::T_STRUCT, 4);
  xfer += this->to_server.write(oprot);
  xfer += oprot->writeFieldEnd();
...
void swap(balancer_proposal_request &a, balancer_proposal_request &b) {
  ...
  swap(a.gpid, b.gpid);
  swap(a.type, b.type);
  swap(a.from_server, b.from_server);
  swap(a.to_server, b.to_server);
  swap(a.__isset, b.__isset);
}
...
balancer_proposal_request::balancer_proposal_request(const balancer_proposal_request& other105) {
  gpid = other105.gpid;
  type = other105.type;
  from_server = other105.from_server;
  to_server = other105.to_server;
  __isset = other105.__isset;
}
balancer_proposal_request& balancer_proposal_request::operator=(const balancer_proposal_request& other106) {
  gpid = other106.gpid;
  type = other106.type;
  from_server = other106.from_server;
  to_server = other106.to_server;
...
void balancer_proposal_request::printTo(std::ostream& out) const {
  ...
  out << "gpid=" << to_string(gpid);
  out << ", " << "type=" << to_string(type);
  out << ", " << "from_server=" << to_string(from_server);
  out << ", " << "to_server=" << to_string(to_server);
  out << ")";
}

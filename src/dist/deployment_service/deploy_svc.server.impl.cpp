...
void deploy_svc_service_impl::stop(dsn_gpid gd) { close_service(gd); }
...
error_code deploy_svc_service_impl::start(dsn_gpid gd)
{
    ...
    _service_dir = dsn_config_get_value_string(
        "deploy.service", "deploy_dir", pdir.c_str(), "where to put temporal deployment resources");
    ...
    for (int i = 0; i < count; i++) {
        std::string cluster_name =
            dsn_config_get_value_string(clusters[i], "name", "", "cluster name");
        ...
        std::string cluster_factory_type = dsn_config_get_value_string(
            clusters[i], "factory", "", "factory name to create the target cluster scheduler");

        auto cluster = ::dsn::utils::factory_store<cluster_scheduler>::create(
            cluster_factory_type.c_str(), PROVIDER_TYPE_MAIN);
...
void deploy_svc_service_impl::on_service_failure(std::shared_ptr<::dsn::dist::deployment_unit> unit,
                                                 ::dsn::error_code err,
...
void deploy_svc_service_impl::download_service_resource_completed(
    error_code err, std::shared_ptr<::dsn::dist::deployment_unit> svc)
{
    if (err != ::dsn::ERR_OK) {
...
void deploy_svc_service_impl::on_service_undeployed(std::string service_name,
                                                    ::dsn::error_code err,
                                                    const std::string &err_msg)
{
...
void deploy_svc_service_impl::on_deploy_internal(const deploy_request &req, /*out*/ deploy_info &di)
{
    ...
    // add to service collections
    {
        ...
        if (it != _services.end()) {
            ...
            _services.insert(
                std::unordered_map<std::string, std::shared_ptr<::dsn::dist::deployment_unit>>::
                    value_type(req.name, svc));
        }
    }
    ...
    if (di.error == ::dsn::ERR_OK) {
        ...
        file::copy_remote_files(req.package_server,
                                source_dir,
                                files,
                                ldir,
                                true,
...
void deploy_svc_service_impl::on_deploy(const deploy_request &req,
                                        ::dsn::rpc_replier<deploy_info> &reply)
{
    deploy_info di;
...
void deploy_svc_service_impl::on_undeploy_internal(const std::string &service_name, error_code &err)
{
    ...
    if (it != _services.end()) {
        ...
        svc->undeployment_callback = [service_name, this](::dsn::error_code err,
                                                          const std::string &err_msg) {
...
void deploy_svc_service_impl::on_undeploy(const std::string &service_name,
                                          ::dsn::rpc_replier<error_code> &reply)
{
    error_code err;
...
void deploy_svc_service_impl::on_get_service_list_internal(const std::string &app_type,
...
void deploy_svc_service_impl::on_get_service_list(const std::string &package_id,
                                                  ::dsn::rpc_replier<deploy_info_list> &reply)
{
    deploy_info_list dlist;
...
void deploy_svc_service_impl::on_get_service_info_internal(const std::string &service_name,
                                                           deploy_info &di)
{
...
void deploy_svc_service_impl::on_get_service_info(const std::string &service_name,
                                                  ::dsn::rpc_replier<deploy_info> &reply)
{
    deploy_info di;
...
void deploy_svc_service_impl::on_get_cluster_list_internal(const std::string &format,
                                                           cluster_list &clist)
{
...
void deploy_svc_service_impl::on_get_cluster_list(const std::string &format,
                                                  ::dsn::rpc_replier<cluster_list> &reply)
{
    cluster_list clist;
...
std::shared_ptr<::dsn::dist::deployment_unit>
deploy_svc_service_impl::get_service(const std::string &name)
{
    ::dsn::service::zauto_read_lock l(_service_lock);
...
std::shared_ptr<deploy_svc_service_impl::cluster_ex>
deploy_svc_service_impl::get_cluster(const std::string &name)
{
    ::dsn::service::zauto_read_lock l(_cluster_lock);

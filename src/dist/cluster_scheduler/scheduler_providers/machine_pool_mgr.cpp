...
machine_pool_mgr::machine_pool_mgr(const char *sec)
{
    ...
    std::string cluster_config_file = dsn_config_get_value_string(
        sec,
        "machine",
        "nodes",
        "the location of the file which lists the host name and user name "
    ...
    dassert(ERR_OK == err,
            "unable to load the cluster config file, pls check your config file again.");
...
error_code machine_pool_mgr::get_machine(const alloca_options &opt,
...
error_code machine_pool_mgr::parse_cluster_config_file(const std::string &cluster_config_file,
                                                       std::vector<std::string> &machine_id)
{

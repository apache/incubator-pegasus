...
public:
    ...
    {
        _resolver =
            replication::partition_resolver::get_resolver(cluster_name, meta_list, app_name);
    }
    ~rrdb_client() { _tracker.cancel_outstanding_tasks(); }

    // ---------- call RPC_RRDB_RRDB_PUT ------------
    // - synchronous
    std::pair<error_code, update_response>
    put_sync(const update_request &args, std::chrono::milliseconds timeout, uint64_t partition_hash)
    {
        return rpc::wait_and_unwrap<update_response>(_resolver->call_op(
    ...
    task_ptr put(const update_request &args,
                 TCallback &&callback,
    ...
    // - synchronous
    std::pair<error_code, update_response> multi_put_sync(const multi_put_request &args,
                                                          std::chrono::milliseconds timeout,
    ...
    template <typename TCallback>
    task_ptr multi_put(const multi_put_request &args,
                       TCallback &&callback,
                       std::chrono::milliseconds timeout,
                       uint64_t request_partition_hash,
                       int reply_thread_hash = 0)
    ...
    // - synchronous
    std::pair<error_code, update_response>
    remove_sync(const blob &args, std::chrono::milliseconds timeout, uint64_t partition_hash)
    {
        return rpc::wait_and_unwrap<update_response>(_resolver->call_op(
            RPC_RRDB_RRDB_REMOVE, args, &_tracker, empty_rpc_handler, timeout, partition_hash));
    }

    // - asynchronous with on-stack blob and update_response
    template <typename TCallback>
    task_ptr remove(const blob &args,
                    TCallback &&callback,
                    std::chrono::milliseconds timeout,
                    uint64_t request_partition_hash,
                    int reply_thread_hash = 0)
    ...
    // - synchronous
    std::pair<error_code, multi_remove_response>
    multi_remove_sync(const multi_remove_request &args,
                      std::chrono::milliseconds timeout,
                      uint64_t partition_hash)
    {
        return rpc::wait_and_unwrap<multi_remove_response>(
    ...
    template <typename TCallback>
    task_ptr multi_remove(const multi_remove_request &args,
                          TCallback &&callback,
                          std::chrono::milliseconds timeout,
                          uint64_t request_partition_hash,
                          int reply_thread_hash = 0)
    ...
    // - synchronous
    std::pair<error_code, incr_response>
    incr_sync(const incr_request &args, std::chrono::milliseconds timeout, uint64_t partition_hash)
    {
        return rpc::wait_and_unwrap<incr_response>(_resolver->call_op(
    ...
    template <typename TCallback>
    task_ptr incr(const incr_request &args,
                  TCallback &&callback,
                  std::chrono::milliseconds timeout,
                  uint64_t request_partition_hash,
                  int reply_thread_hash = 0)
    ...
    // - synchronous
    std::pair<error_code, check_and_set_response>
    check_and_set_sync(const check_and_set_request &args,
                       std::chrono::milliseconds timeout,
                       uint64_t partition_hash)
    {
        return rpc::wait_and_unwrap<check_and_set_response>(
    ...
    template <typename TCallback>
    task_ptr check_and_set(const check_and_set_request &args,
                           TCallback &&callback,
                           std::chrono::milliseconds timeout,
                           uint64_t request_partition_hash,
                           int reply_thread_hash = 0)
    ...
    // - synchronous
    std::pair<error_code, check_and_mutate_response>
    check_and_mutate_sync(const check_and_mutate_request &args,
                          std::chrono::milliseconds timeout,
                          uint64_t partition_hash)
    {
        return rpc::wait_and_unwrap<check_and_mutate_response>(
    ...
    template <typename TCallback>
    task_ptr check_and_mutate(const check_and_mutate_request &args,
                              TCallback &&callback,
                              std::chrono::milliseconds timeout,
                              uint64_t request_partition_hash,
                              int reply_thread_hash = 0)
    ...
    // - synchronous
    std::pair<error_code, read_response>
    get_sync(const blob &args, std::chrono::milliseconds timeout, uint64_t partition_hash)
    {
        return rpc::wait_and_unwrap<read_response>(_resolver->call_op(
            RPC_RRDB_RRDB_GET, args, &_tracker, empty_rpc_handler, timeout, partition_hash));
    }

    // - asynchronous with on-stack blob and read_response
    template <typename TCallback>
    task_ptr get(const blob &args,
                 TCallback &&callback,
                 std::chrono::milliseconds timeout,
                 uint64_t request_partition_hash,
                 int reply_thread_hash = 0)
    ...
    // - synchronous
    std::pair<error_code, multi_get_response> multi_get_sync(const multi_get_request &args,
                                                             std::chrono::milliseconds timeout,
                                                             uint64_t partition_hash)
    {
        return rpc::wait_and_unwrap<multi_get_response>(_resolver->call_op(
    ...
    template <typename TCallback>
    task_ptr multi_get(const multi_get_request &args,
                       TCallback &&callback,
                       std::chrono::milliseconds timeout,
                       uint64_t request_partition_hash,
                       int reply_thread_hash = 0)
    ...
    // - synchronous
    std::pair<error_code, batch_get_response> batch_get_sync(const batch_get_request &args,
                                                             std::chrono::milliseconds timeout,
                                                             uint64_t partition_hash)
    {
        return rpc::wait_and_unwrap<batch_get_response>(_resolver->call_op(
    ...
    template <typename TCallback>
    task_ptr batch_get(const batch_get_request &args,
                       TCallback &&callback,
                       std::chrono::milliseconds timeout,
                       uint64_t request_partition_hash,
                       int reply_thread_hash = 0)
    ...
    // - synchronous
    std::pair<error_code, count_response>
    sortkey_count_sync(const blob &args, std::chrono::milliseconds timeout, uint64_t partition_hash)
    {
        return rpc::wait_and_unwrap<count_response>(_resolver->call_op(RPC_RRDB_RRDB_SORTKEY_COUNT,
                                                                       args,
                                                                       &_tracker,
                                                                       empty_rpc_handler,
                                                                       timeout,
    ...
    // - asynchronous with on-stack blob and count_response
    template <typename TCallback>
    task_ptr sortkey_count(const blob &args,
                           TCallback &&callback,
                           std::chrono::milliseconds timeout,
                           uint64_t request_partition_hash,
                           int reply_thread_hash = 0)
    ...
    // - synchronous
    std::pair<error_code, ttl_response>
    ttl_sync(const blob &args, std::chrono::milliseconds timeout, uint64_t partition_hash)
    {
        return rpc::wait_and_unwrap<ttl_response>(_resolver->call_op(
            RPC_RRDB_RRDB_TTL, args, &_tracker, empty_rpc_handler, timeout, partition_hash));
    }

    // - asynchronous with on-stack blob and ttl_response
    template <typename TCallback>
    task_ptr ttl(const blob &args,
                 TCallback &&callback,
                 std::chrono::milliseconds timeout,
                 uint64_t request_partition_hash,
                 int reply_thread_hash = 0)
    ...
    // - synchronous
    std::pair<error_code, scan_response>
    get_scanner_sync(const get_scanner_request &args,
                     std::chrono::milliseconds timeout,
                     uint64_t partition_hash,
                     pegasus::optional<rpc_address> server_addr = pegasus::none)
    {
        return rpc::wait_and_unwrap<scan_response>(_resolver->call_op(RPC_RRDB_RRDB_GET_SCANNER,
                                                                      args,
                                                                      &_tracker,
                                                                      empty_rpc_handler,
                                                                      timeout,
    ...
    template <typename TCallback>
    task_ptr get_scanner(const get_scanner_request &args,
                         TCallback &&callback,
                         std::chrono::milliseconds timeout,
                         uint64_t request_partition_hash,
                         int reply_thread_hash = 0)
    ...
    {
        return rpc::wait_and_unwrap<scan_response>(_resolver->call_op(
    ...
    template <typename TCallback>
    task_ptr scan(const scan_request &args,
                  TCallback &&callback,
                  std::chrono::milliseconds timeout,
                  uint64_t request_partition_hash,
                  int reply_thread_hash = 0)
    ...
    template <typename TCallback>
    task_ptr duplicate(duplicate_rpc &rpc, TCallback &&callback, task_tracker *tracker)
    {
        return rpc.call(_resolver, tracker, std::forward<TCallback &&>(callback));
    }

private:
    replication::partition_resolver_ptr _resolver;
